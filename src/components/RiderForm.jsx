...
function RiderForm({ onBack, editingRiderId = null }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-dark-950 via-dark-900 to-dark-800 p-4">
      ...
      {/* Tabs Navigation */}
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex justify-between bg-dark-900/80 backdrop-blur-sm rounded-2xl p-3 border border-dark-800/50 shadow-xl">
          {tabs.map((tab) => (
            <button
              key={tab.id}
            ...
            >
              ...
              <span className="hidden sm:inline font-medium text-xs">{tab.title}</span>
            </button>
          ))}
        </div>
      </div>
